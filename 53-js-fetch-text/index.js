...
const getResponse = (url) => {
    fetch(url)
        .then(response => response.text())
            .then((numberFact) => result.innerHTML = `<h3>${numberFact}</h3>`)
            
            // retrieve text using fetch
            //print element in .result
            // clear input and hide .result element
           
}

form.addEventListener("submit", function(){ getResponse(url) });

// add event listener and clear input